    ...
    'render: loop {
        ...
        for statement in form.StatementListHead.iter() {
            ...
            for option in statement.OptionListHead.iter() {
                ...
                if let Some(op) = option.OptionOpCode() {
                    ...
                    let prompt = ui.font.render(&string(op.Option).unwrap_or(String::new()), font_size);
        ...
        'display: loop {
            ...
            let draw_options_box = |display: &mut Display, x: i32, mut y: i32, element: &Element| {
                ...
                if y > start_y {
                    ui.draw_pretty_box(display, x, start_y, w, (y - start_y - margin_tb) as u32, false);
                }
            ...
            if let Some(ref title) = title_opt {
                ...
                ui.draw_text_box(&mut display, x, y, &rendered, false, false);
            ...
            // Draw footer
            {
                ...
                let mut render_hotkey_help = |help: &str| {
                    ...
                    ui.draw_text_box(&mut display, x, bottom_y, &rendered, false, false);
                    i += 1;
                };
            ...
            if element_start > 0 {
                ...
                ui.draw_text_box(&mut display, (display_w - arrow.width()) as i32 - margin_lr, y, &arrow, false, false);
            }
            ...
            for i in element_start..(element_start + max_form_elements) {
                if let Some(element) = elements.get(i) {
                    ...
                    let h = {
                        // TODO: Do not render in drawing loop
                        let rendered = ui.font.render(&element.prompt, font_size);
                        ui.draw_text_box(&mut display, margin_lr, y, &rendered, highlighted && ! editing, highlighted && ! editing);
                        rendered.height() as i32
                    };
                    ...
                    if element.list {
                        ...
                        ui.draw_text_box(&mut display, x, y, &option.prompt, true, highlighted && editing);
            ...
            if elements.len() > max_form_elements && element_start < elements.len() - max_form_elements {
                ...
                let arrow = ui.font.render("↓", help_font_size);
                ui.draw_text_box(&mut display, (display_w - arrow.width()) as i32 - margin_lr, bottom_y - arrow.height() as i32 - margin_tb * 2, &arrow, false, false);
            }
